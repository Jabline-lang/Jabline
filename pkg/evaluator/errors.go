...
// ErrorContext provides context information for better error messages
...
// EnhancedError represents an error with additional context
...
// newNameError creates a name/identifier error
...
// newIndexError creates an index-related error
...
// newKeyError creates a key-related error for hash maps
...
// newArithmeticError creates an arithmetic-related error
...
// newSyntaxError creates a syntax-related error
...
// newFunctionError creates a function-related error
...
// Enhanced error creators that replace the simple newError function

// newErrorWithContext creates an error with context information
func newErrorWithContext(message string, node ast.Node) *object.Error {
	context := &ErrorContext{}

	// Try to extract context from the node if possible
	if node != nil {
		// For now, we don't have line/column info in AST nodes
...
// newArgumentError creates an argument-related error
...
// Helper function to create context from token (if available in the future)
...
// getOperatorName returns a human-readable name for operators
